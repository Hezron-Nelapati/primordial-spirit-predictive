...
- **Centroid classification** (Phase 14): loads `all-MiniLM-L6-v2` + `data/centroids.json` at startup; calls `centroid_classify()` using same POS tag sets and blended formula as `classify_query.py` — symmetric training/runtime tags. Graceful fallback to `mock_classify()` if packages absent.
...
`implementation_plan.md §1` requires the Python pipeline to classify symmetrically in both training and runtime. Previously `v2_ingest.py` used keyword heuristics (`mock_classify`) while runtime used the centroid model — edge tags were inconsistent.

- [x] `v2_ingest.py`: `centroid_classify()` added — mirrors `classify_query.py` POS tag sets (`INTENT_TAGS`, `TONE_TAGS`, `DOMAIN_TAGS`) and 0.7/0.3 blended distance formula exactly. Model loaded once in `main()`. Fallback to `mock_classify()` preserved.
- [x] `v3_ingest.py`: `batch_classify()` added — encodes all sentences in a paragraph as a single batch (`batch_size=64`) for speed on the massive Wikipedia corpus. Graceful fallback to `statement/neutral/general`.
...
1. **V3 data absent** — both `corpus_v3_massive.txt` and `v3_graph_edges.json` must be generated by running `python/download_corpus.py` then `python/v3_ingest.py`; the Rust load path is ready and waiting. V3 ingest now uses batch centroid classification when model is available.
...
5. **Activate V3 pipeline**: `cd python && python download_corpus.py && python v3_ingest.py` — Rust will auto-load on next run. V3 now classifies via centroid model (batch) when available.
