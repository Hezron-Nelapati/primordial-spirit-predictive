...
use spse_predictive::ingest::{ingest_sentence, ingest_text, ingest_v2_rows, GraphStats, V2JsonData};
...
fn make_row(tokens: &[&str], intent: &str, domain: &str, dated: Option<u16>) -> V2JsonData {
    V2JsonData {
        text:     tokens.join(" "),
...
#[test]
fn test_ingest_v2_rows_basic_node_and_edge_creation() {
    let mut graph = WordGraph::new();
    ingest_v2_rows(&mut graph, vec![make_row(&["hello", "world"], "statement", "general", None)]);
...
#[test]
fn test_ingest_v2_rows_reinforces_identical_edges() {
    let mut graph = WordGraph::new();
    let rows = vec![
        make_row(&["server", "online"], "statement", "tech", None),
        make_row(&["server", "online"], "statement", "tech", None),
    ];
    ingest_v2_rows(&mut graph, rows);
...
#[test]
fn test_ingest_v2_rows_different_domains_create_distinct_edges() {
    let mut graph = WordGraph::new();
    let rows = vec![
...
#[test]
fn test_ingest_v2_rows_different_dated_create_distinct_edges() {
    let mut graph = WordGraph::new();
    let rows = vec![
...
#[test]
fn test_ingest_v2_rows_populates_node_positions() {
    let mut graph = WordGraph::new();
    ingest_v2_rows(&mut graph, vec![make_row(&["quantum", "physics"], "explain", "science", None)]);
